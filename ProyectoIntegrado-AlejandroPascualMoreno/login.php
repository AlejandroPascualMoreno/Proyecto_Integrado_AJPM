...
if (isset($_POST['user']) && isset($_POST['pass'])) {
    ...
    $statement = $conexion->prepare('SELECT id_vendedor FROM vendedores WHERE correo = :user AND contrasena = :pass');
    ...
    if ($resultado) {
        ...
        $statement = $conexion->prepare('SELECT id_comprador FROM compradores WHERE correo = :user AND contrasena = :pass');
        $statement->execute(array(
            ':user' => $usuario,
            ':pass' => $password,
        ));
